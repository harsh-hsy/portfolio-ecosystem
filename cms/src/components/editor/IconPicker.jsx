...
function getPopoverPosition(trigger, { compact }) {
  ...
  const left = Math.min(Math.max(rect.left, VIEWPORT_GAP), window.innerWidth - width - VIEWPORT_GAP)
...
function IconPicker({
  ...
  return (
    <div className={`form-group icon-picker ${error ? 'form-group--error' : ''}`} ref={rootRef}>
