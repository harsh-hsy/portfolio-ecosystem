...
import { getProjectDetailsContent } from '../content/contentSelectors.js'
import { getProjectBySlug, getRelatedProjects } from '../utils/projects.js'
      ...
      <div className="container">
            ...
            <div className="project-actions detail-actions">
              {project.live ? (
                <a href={project.live} target="_blank" rel="noreferrer">
                  <FiExternalLink /> {ui.liveDemoLabel}
                </a>
              ) : null}
              {project.github ? (
                <a href={project.github} target="_blank" rel="noreferrer">
                  <FiGithub /> {ui.githubLabel}
                </a>
              ) : null}
            </div>
          </Reveal>
        ...
        <div className="detail-grid">
          {ui.detailCards.map((card) => {
            const body =
              card.type === 'list'
                ? project[card.field].join(ui.listSeparator)
                : project[card.field]
            return (
              <Reveal as="article" className="detail-card" key={card.label}>
                <h2>{card.label}</h2>
                <p>{body}</p>
              </Reveal>
            )
        ...
        <Reveal className="screenshot-grid">
          {project.images.map((image, index) => (
            <img
              key={image}
              src={image}
              alt={`${project.shortTitle} ${ui.screenshotLabel} ${index + 1}`}
              loading="lazy"
            />
          ))}
        </Reveal>
