...
export default function Hero({ entranceReady }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () => setIndex((value) => (value + 1) % rotatingRoleCount),
      1800,
    )
    ...
    <section id="home" className="hero-section section">
      ...
      >
        <motion.div className="hero-copy" variants={copyEntrance}>
          ...
          <h1>
            {content.intro} <span>{profile.name}</span>
          </h1>
          ...
          <div className="hero-actions">
            <MagneticButton href={profile.resume} target="_blank" className="primary">
              <FiDownload /> {content.primaryAction}
            </MagneticButton>
            <MagneticButton href="#projects" className="secondary">
              <FiArrowDown /> {content.secondaryAction}
            </MagneticButton>
            <MagneticButton href="#contact" className="ghost">
              <FiSend /> {content.contactAction}
            </MagneticButton>
          </div>
        ...
        <motion.div className="hero-visual" variants={imageEntrance}>
          <div className="portrait-shell">
            ...
            />
            <motion.div
              className="orbit-card top"
              animate={reduceEffects ? undefined : { y: [0, -10, 0] }}
              transition={reduceEffects ? undefined : { repeat: Infinity, duration: 4 }}
            >
              <LocationIcon /> {profile.location}
            </motion.div>
            <motion.div
              className="orbit-card bottom"
              animate={reduceEffects ? undefined : { y: [0, 10, 0] }}
              transition={reduceEffects ? undefined : { repeat: Infinity, duration: 4.5 }}
            >
              {content.orbitRole}
            </motion.div>
          </div>
