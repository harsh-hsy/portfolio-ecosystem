...
function normalizePortfolioUrl(value, fallback) {
  ...
  return normalized === "https://harsh-hsy.onrender.com"
    ? fallback
    : normalized;
}
...
async function fetchPortfolio(apiBaseUrl) {
  ...
  try {
    const response = await fetch(
      `${apiBaseUrl.replace(/\/$/, "")}/api/portfolio`,
      {
...
function metadataFrom(content, defaults) {
  ...
  const siteUrl = normalizePortfolioUrl(
    identity.portfolioUrl || seo.siteUrl || defaultSiteUrl,
    defaultSiteUrl,
  );
...
function meta(attrs) {
  return {
    tag: "meta",
    attrs: { ...attrs, "data-build-seo": "true" },
    injectTo: "head",
  };
}
  ...
  return {
    ...
    async transformIndexHtml(html) {
      ...
      if (metadata.bingVerification) {
        tags.push(
          meta({ name: "msvalidate.01", content: metadata.bingVerification }),
        );
      }
