...
export default function CommandPalette() {
  ...
  const {
    ...
    projects,
    ui,
  } = getCommandPaletteContent(contentState?.portfolio)
  ...
  const actions = useMemo(
    () => [
      ...configuredActions.map((action) => ({
        ...
        run: () => {
          ...
          if (action.type === 'externalProfile') {
            const url =
              action.target === 'resume' ? getResumeUrl(profile.resume) : profile[action.target]
      ...
      ...projects.map((project) => ({
        label: `${ui.projectPrefix} ${project.shortTitle}`,
  ...
  return (
    <>
      <button
        ...
        aria-label={ui.triggerLabel}
      >
        <FiCommand />
        <span>{ui.shortcutLabel}</span>
      </button>
      <AnimatePresence>
        {open && (
          <motion.div
