...
import { PASSWORD_RULES, SECURITY_TIPS } from '../../data/password'
...
function PasswordSection({ isLoading = false, passwordChangedAt = null }) {
  ...
  const requirements = useMemo(() => {
    return PASSWORD_RULES.map((rule) => ({
      ...rule,
      valid: rule.validator(formData.newPassword),
    }))
  }, [formData.newPassword])

  const passwordsMatch =
    formData.confirmPassword.length === 0 ? null : formData.newPassword === formData.confirmPassword

  const canSave =
    formData.currentPassword.trim().length > 0 &&
    requirements.every((rule) => rule.valid) &&
    passwordsMatch === true &&
    !isSaving
  ...
  function validateForm() {
    ...
    if (!requirements.every((rule) => rule.valid)) {
  ...
  return (
    <section className="panel account-section">
      <h2 className="account-section__title">Password & Security</h2>
      ...
      {status.message && <PanelStatus message={status.message} type={status.type} />}
